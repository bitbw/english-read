...
export function getTierLabel(id: ReadingTierId): string {
  return READING_TIERS.find((t) => t.id === id)?.label ?? id;
}

/** 拼进 LLM 的分级列表说明 */
export function buildTierPromptBlock(): string {
  return READING_TIERS.map((t) => `- ${t.id}: ${t.label} — ${t.llmHint}`).join("\n");
}
