...
export function StatsPageClient({ title, backLabel }: StatsPageClientProps) {
  ...
  const buildFetchUrl = useCallback((mode: RangeMode, start?: string, end?: string) => {
    if (mode === "custom" && start && end) {
      return `/api/stats/study?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`;
    }
    return `/api/stats/study?days=${mode}`;
  }, []);
